			...
			<table class="large-12 data-table">
				...
				<tbody>
					...
					foreach ($query as $data) 
					{
						...
						anchor("paginas/editar/".$data->id_midia,' ',array('class'=>'table-actions table-edit','title'=>'Editar')).
						anchor("paginas/excluir/".$data->id_midia,' ',array('class'=>'table-actions table-delet deletareg','title'=>'Excluir')));
					}
					?>
				</tbody>				
			</table>
	...
	case 'editar':
		...
		<div class="large-12 columns">
			<?php 
				if (stats_user())
				{
					$query =  $this->paginas->get_by_id($idmidia)->row();
